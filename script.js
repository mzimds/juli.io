...
const DOM = {
    ...
    filterPatient: document.getElementById('filterPatient'),
...
function adicionarEvento(tipo, setorId, medico, pacienteId, pacienteNome, detalhes) {
    const evento = {
        id: Date.now(),
        ...
        setorId,
        medico,
        setorNome: dados.setores.find(s => s.id === setorId)?.nome || 'N/A',
        pacienteId,
        pacienteNome,
        detalhes
    };
    state.eventosSistema.push(evento);
}
...
function init() {
    ...
    // Preencher o filtro de setor no histórico
    dados.setores.forEach(setor => {
        const option = document.createElement('option');
        option.value = setor.id;
        option.textContent = setor.nome;
        DOM.filterSector.appendChild(option);
    });
...
function selectPaciente(pacienteId) {
    ...
    // Marcar como paciente selecionado
...
function setupEventListeners() {
    ...
    DOM.btnSavePaciente.addEventListener('click', () => {
        ...
        if (setor) {
            ...
            adicionarEvento(
                ...
                novoPaciente.id,
                novoPaciente.nome,
                `Paciente admitido no setor ${setor.nome}`
            );
    ...
    const filterInputs = [
        'filterStartDate', 'filterEndDate', 'filterSector', 
        'filterDoctor', 'filterEventType', 'filterPatient'
    ];
...
function deletePaciente() {
    ...
    for (const setor of dados.setores) {
        ...
        if (pacienteIndex !== -1) {
            const paciente = setor.pacientes[pacienteIndex];
    ...
    for (const setor of dados.setores) {
        ...
        if (paciente && paciente.pendencias) {
            ...
            if (pendencia) {
                ...
                // Adicionar evento de resolução de pendência
                adicionarEvento(
                    'resolucao_pendencia',
                    setor.id,
                    state.currentDoctor,
                    paciente.id,
                    paciente.nome,
                    `Pendência resolvida: ${pendencia.titulo}`
                );
...
function loadHistoryData() {
    const filters = {
        ...
        doctor: DOM.filterDoctor.value,
        eventType: DOM.filterEventType.value,
        patient: DOM.filterPatient.value.toLowerCase()
    };
    
    const filteredData = state.eventosSistema.filter(evento => {
        // Filtro por período
        if (filters.startDate && new Date(evento.timestamp) < new Date(filters.startDate)) {
            return false;
        }
        
        if (filters.endDate && new Date(evento.timestamp) > new Date(filters.endDate)) {
            return false;
        }
        
        // Filtro por setor
        if (filters.sector && evento.setorId != filters.sector) {
            return false;
        }
        
        // Filtro por médico
        if (filters.doctor && evento.medico !== filters.doctor) {
            return false;
        }
        
        // Filtro por tipo de evento
        if (filters.eventType && evento.tipo !== filters.eventType) {
            return false;
        }
        
        // Filtro por paciente
        if (filters.patient && !evento.pacienteNome.toLowerCase().includes(filters.patient)) {
            return false;
        }
...
function renderHistoryTable(data) {
    ...
    data.forEach(evento => {
        ...
        row.innerHTML = `
            ...
            <td>${formatEventType(evento.tipo)}</td>
            <td>${evento.setorNome}</td>
            <td>${evento.medico}</td>
            <td>${evento.pacienteNome}</td>
...
function formatEventType(tipo) {
    const tipos = {
        ...
        'alta': 'Alta',
        'admissao': 'Admissão',
        'resolucao_pendencia': 'Resolução Pendência',
        'exclusao': 'Exclusão'
    };
    return tipos[tipo] || tipo;
}

// Função para remover acentos (para o CSV)
...
function exportToCSV() {
    let csv = 'Data/Hora,Tipo,Setor,Médico,Paciente,Detalhes\n';
    
    state.eventosSistema.forEach(evento => {
        csv += `"${formatDateTime(evento.timestamp)}",${removeAcentos(formatEventType(evento.tipo))},${removeAcentos(evento.setorNome)},${removeAcentos(evento.medico)},${removeAcentos(evento.pacienteNome)},"${removeAcentos(evento.detalhes)}"\n`;
    });
    ...
    link.setAttribute('download', 'historico_completo.csv');
...
function exportToJSON() {
    const dataStr = JSON.stringify(state.eventosSistema, null, 2);
    const dataUri = `data:application/json;charset=utf-8,${encodeURIComponent(dataStr)}`;
    
    const link = document.createElement('a');
    link.setAttribute('href', dataUri);
    link.setAttribute('download', 'historico_completo.json');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
